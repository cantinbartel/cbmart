...
const PlaceOrderScreen = () => {
    ...
    cart.totalPrice = round(Number(cart.itemsPrice) + Number(cart.shippingPrice))
                        ...
                        {cart.cartItems.length === 0 ?
                            ...
                            cart.cartItems.map((item, index) => (
                                <React.Fragment key={item.product}>
                                    <div
                                        className={`flex flex-col lg:flex-row justify-start items-start font-semibold m-4 ${index === 0 ? 'mt-8' : ''}`}>
